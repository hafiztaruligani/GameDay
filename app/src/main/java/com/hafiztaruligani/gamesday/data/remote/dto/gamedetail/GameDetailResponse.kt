...
data class GameDetailResponse(
	...
	fun toGameDetailEntity(favorite: Boolean) = GameDetailEntity(
		id = id?:0,
		name = name?:"-",
		releaseDate = released?:"-",
		rating = if (rating!=null && rating > 0.0) rating.toString() else "-",
		image = backgroundImage?:"-",
		description = description?:"-",
		company = getCompany(),
		playCount = addedByStatus?.owned?: addedByStatus?.toplay?:0,
		favorite = favorite
	)

	private fun getCompany(): String {
		val publisher = publishers?.firstOrNull { it?.name != null }?.name
		val developer = developers?.firstOrNull { it?.name != null }?.name
		return when{
